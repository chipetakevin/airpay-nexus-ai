...
// Add logo to PDF
const addLogoToPDF = (doc: jsPDF, x: number, y: number, width: number = 40, height: number = 12) => {
  // Create a simple logo representation since we can't embed the actual image file
  // This creates a blue rounded rectangle with "DM" text to represent the Divinely Mobile logo
  ...
  doc.text('DM', x + width/2 - 6, y + height/2 + 2);
...
export const generateCustomerReport = (customer: Customer, customerTransactions: Transaction[], toast: any) => {
  ...
  // Add logo
  addLogoToPDF(doc, 20, 15);
  
  // Header
  doc.setFontSize(20);
  doc.text('OneCard Customer Report', 70, 25);
  doc.setFontSize(12);
  doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 70, 35);
  ...
  if (customerTransactions.length > 0) {
    ...
    customerTransactions.slice(0, 10).forEach((tx, index) => {
      if (yPos > 270) {
        doc.addPage();
        addLogoToPDF(doc, 20, 15); // Add logo to new page
        yPos = 35;
      }
  ...
  doc.text('Generated by OneCard Platform', 20, doc.internal.pageSize.height - 10);
...
export const generateMasterReport = (customers: Customer[], transactions: Transaction[], toast: any) => {
  ...
  // Header
  doc.setFontSize(20);
  doc.text('OneCard Master Report', 70, 25);
  doc.setFontSize(12);
  doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 70, 35);
