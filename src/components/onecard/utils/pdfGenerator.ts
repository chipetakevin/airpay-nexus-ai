...
// Add Divinely Mobile logo to PDF with consistent branding
const addDivinelyMobileLogo = (doc: jsPDF, x: number, y: number, width: number = 50, height: number = 16) => {
...
export const generateCustomerReport = (customer: Customer, customerTransactions: Transaction[], toast: any) => {
  ...
  // Add Divinely Mobile logo
  addDivinelyMobileLogo(doc, 20, 15);
  ...
  if (customerTransactions.length > 0) {
    ...
    customerTransactions.slice(0, 10).forEach((tx, index) => {
      if (yPos > 270) {
        doc.addPage();
        addDivinelyMobileLogo(doc, 20, 15); // Add logo to new page
        yPos = 35;
      }
  ...
  doc.text('Generated by Divinely Mobile OneCard Platform', 20, doc.internal.pageSize.height - 10);
  
  doc.save(`Divinely_Mobile_OneCard_Report_${customer.firstName}_${customer.lastName}_${new Date().toISOString().split('T')[0]}.pdf`);
