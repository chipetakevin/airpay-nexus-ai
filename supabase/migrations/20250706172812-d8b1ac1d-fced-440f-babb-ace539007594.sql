-- Add South African payroll compliance calculation functions
...
CREATE OR REPLACE FUNCTION public.calculate_paye(
  ...
  employee_age INTEGER DEFAULT 35,
...
BEGIN
  ...
  -- Calculate final PAYE (ensure not negative)
...
-- Function to calculate SDL (Skills Development Levy)
...
-- Function to process complete payroll calculations for an employee
...
BEGIN
  ...
  -- Calculate PAYE (assuming age 35 for demo)
  paye_amount := public.calculate_paye(gross_salary * 12, 35, 0, 0);
  ...
  -- Insert payroll calculation (payslip)
...
CREATE TRIGGER payroll_run_status_trigger 
    BEFORE UPDATE ON payroll_runs 
    FOR EACH ROW 
    EXECUTE FUNCTION auto_calculate_payroll();