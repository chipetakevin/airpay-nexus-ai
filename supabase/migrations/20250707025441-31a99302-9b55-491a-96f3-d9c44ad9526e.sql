...
BEGIN
  ...
  -- Determine role based on user type (using correct enum values)
  CASE user_type_meta
    WHEN 'admin' THEN user_role_type := 'admin';
    WHEN 'vendor' THEN user_role_type := 'vendor';
    WHEN 'customer' THEN user_role_type := 'customer';
    ELSE user_role_type := 'customer'; -- Default to customer role
  END CASE;
